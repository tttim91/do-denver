        ...
        .then(function (places) {
          return Promise.all([
            // Inserts seed entries
              knex('comment').insert({comment_body:"This is a test Comment from Mairin", place_rating: 5, client_id: find.findClient('mabaile', clients), place_id:find.findLocation('Amethyst', places)}),
              knex('comment').insert({comment_body:"This is a test comment from Steven", place_rating: 3, client_id:find.findClient('wolfshark', clients), place_id:find.findLocation('Downpours', places)}),
